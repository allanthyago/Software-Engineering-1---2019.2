...
public class CadastroObservaçõesEdProf extends JDialog {
	 ...
	 * @param string 
	 */
	public CadastroObservaçõesEdProf(int prof, String string, int codigo) {
		...
		setIconImage(Toolkit.getDefaultToolkit().getImage(CadastroObservaçõesEdProf.class.getResource("/Imagem/Senai.png")));
			...
			public void windowClosing(WindowEvent e) {
				int reply = JOptionPane.showConfirmDialog(null, "Deseja cancelar?", "Confirma Exit", JOptionPane.YES_NO_OPTION);
				  if (reply == JOptionPane.YES_OPTION)
				  {
					...
					TabelaObsEdirProf tp = new TabelaObsEdirProf(p.getCodigo(), u.getNome());
					dispose();
					tp.setVisible(true);
				  }
				  else {
		...
		btnLimpar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				int reply = JOptionPane.showConfirmDialog(null, "Deseja limpar janela?", "Cadastro de Observações", JOptionPane.YES_NO_OPTION);
				  ...
				  {
				    System.out.println("Confirmou");
				    CadastroObservaçõesEdProf novaJanela = new CadastroObservaçõesEdProf(prof, string, codigo);
					dispose();
