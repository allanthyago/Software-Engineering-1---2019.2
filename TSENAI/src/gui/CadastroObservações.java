...
public class CadastroObservações extends JDialog {
	 ...
	 * @param string 
	 */
	public CadastroObservações(DefaultTableModel modelTD, int prof, String string, int codigo) {
		p=pdao.getProfessor(prof);
		u=udao.getUsuario(codigo);
		setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);
		setTitle("TSENAI   |    Cadastro de Observa\u00E7\u00F5es");
			...
			public void actionPerformed(ActionEvent arg0) {
				int reply = JOptionPane.showConfirmDialog(null, "Deseja limpar janela?", "Cadastro de Observações", JOptionPane.YES_NO_OPTION);
				  if (reply == JOptionPane.YES_OPTION)
				  {
				    ...
				    CadastroObservações novaJanela = new CadastroObservações(modelTD, prof, string, codigo);
					dispose();
